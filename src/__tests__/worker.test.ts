...
describe("Cloudflare Worker entrypoint", () => {
  ...
  it("handles MCP initialize", async () => {
    ...
    expect(body.result?.serverInfo?.name).toBe("salesbuildr-mcp");
  });
  ...
  it("lists all tools without credentials", async () => {
    ...
    expect(names).toContain("salesbuildr_navigate");
    expect(names).toContain("salesbuildr_status");
    expect(names).toContain("salesbuildr_companies_list");
  ...
  it("returns a graceful error for a credential-requiring tool when unconfigured", async () => {
    const res = await mcp({
      ...
      id: 3,
      method: "tools/call",
      params: { name: "salesbuildr_companies_list", arguments: {} },
    });
    expect(res.status).toBe(200);
    ...
    expect(body.result?.content?.[0]?.text).toMatch(/api key|credentials/i);
  });
